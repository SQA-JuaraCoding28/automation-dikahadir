...
import com.hadir.pages.IzinPulangCepatPage;
...
public class IzinPulangCepatSteps {
    ...
    private IzinPulangCepatPage izinPage = new IzinPulangCepatPage(driver);
    ...
    public void user_is_logged_in_and_on_the_izin_pulang_cepat_page() {
        // LoginPage steps run first, so we are logged in.
    ...
    public void the_table_should_show_records_filtered_by_dates() {
        // Just verify the table loaded and didn't crash.
    ...
    public void the_table_should_show_the_previous_page_of_records() {
        String newPaginationText = izinPage.getPaginationText();
        // Since we went next and then previous, we should check it matches or changed
    ...
    public void the_table_should_show_at_most_records(Integer limit) {
        ...
        // If "no data" message is one of the rows, or table is empty, rowCount could be 1 with text "No data"
        if (izinPage.isNoDataDisplayed()) {
            rowCount = 0;
        }
